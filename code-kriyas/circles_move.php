<html>
	<head>
		<title>Template.</title>
		<script type="text/javascript" src="//use.typekit.net/gle3hly.js"></script>
		<script type="text/javascript">try{Typekit.load();}catch(e){}</script>
		<link rel="stylesheet" type="text/css" href="../_css/topLevelStyle.css">
		<script type="text/javascript" src="http://code.createjs.com/easeljs-0.7.1.min.js"></script>
		<script type="text/javascript" src="_js/circles-moving.js"></script>
	</head>
	
	<body onload="init();">
		<?php
			include '../_includes/kriyas-header.php';
		?>

		<div id="kriya">
			<canvas id="canvas" width="1000" height="500">
			better browsers are better!  with a modern browser you would be seeing some primitive animations here. 
			</canvas>
			<h3>
				They follow their own simple math.
			</h3>
			<p>
			One wonders what it's all worth. Snow falls.
			</p>
			
		</div><!-- kriya -->
</body>
</html>